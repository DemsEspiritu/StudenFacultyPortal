  ...
  <x-notify::notify />
    ...
    <div class="wrapper">
        ...
        <div class="card-body">
            ...
            <div class="col-md-12">
                    <div class="alert alert-info mb-0" role="alert">
                        <p class="mb-0">You are currently viewing on  <b>{{$attendanceData->first()->attendance_date}}</b></b></p>
                   </div>
               </div>
            ...
            <input type="date" name="attendance_date" id="attendance_date"  value="{{ $attendanceData->first()->attendance_date }}" required>
</div> -->
    ...
    @foreach($students as $key => $student)
        <tr>
            ...
            <td class="text-center bg-light">
                ...
                <input type="hidden" name="attendances[{{$key}}][attendance_date]" value="{{ $attendanceData->first()->attendance_date }}">
                @php
                    $studentAttendance = $attendanceData->where('student_id', $student->id)->first();
                @endphp
                <input {{ optional($studentAttendance)->status === 'present' ? 'checked' : '' }} value="present"
                    class="present mr-2" type="radio" name="attendances[{{$key}}][status]" id="{{$student->id}}_1" disabled>
                <input {{ optional($studentAttendance)->status === 'absent' ? 'checked' : '' }} value="absent"
                    class="absent mr-2" type="radio" name="attendances[{{$key}}][status]" id="{{$student->id}}_2" disabled>
                <input {{ optional($studentAttendance)->status === 'tardy' ? 'checked' : '' }} value="tardy"
                    class="tardy mr-2" type="radio" name="attendances[{{$key}}][status]" id="{{$student->id}}_3" disabled>
            </td>
        </tr>
    @endforeach
</tbody>
