    ...
    @foreach($data['getRecord'] as $value)
        @if($value->status != 'Canceled')
            <tr>
                ...
                <td class="text-center">{{ \Carbon\Carbon::parse($value->created_at)->format('F j, Y h:i A') }}</td>
                <td class="text-center">{{ $value->status }}</td>

                <!-- Conditionally display buttons based on status -->
                <td class="text-center">
                    @if($value->status != 'File Release' && $value->status != 'Decline')
                        <!-- Approved -->
                        <a href="#" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#Delete{{ $value->form_id }}">Approved</a>

                        <!-- Declined -->
                        <a href="{{ url('faculty/request/list/decline'.$value->form_id) }}" class="btn btn-danger btn-sm">Declined</a>
                    @endif
                </td>
